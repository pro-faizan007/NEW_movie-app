...
const Navbar = () => {
    ...
    const { searchData, newSerachData, searchBarShow, setSearchBarShowOnNav, isAuthenticated, handleOnChange, userName, userInput } = context
    const signOutClick = () => {
    ...
    return (
        <div className='navbar'>
            ...
            {isAuthenticated ? <Button onClick={signOutClick} style={{ color: "azure" }} variant="primary">Sign Out</Button> : ''}
            {isAuthenticated ? <Button style={{ color: "azure" }} variant="primary">{userInput.name}</Button> : ''}
        </div>
    )
}
